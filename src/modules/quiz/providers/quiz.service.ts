...
export class QuizService {
  ...
  async submitQuiz(submitQuizDto: SubmitQuizDto): Promise<QuizResult> {
    try {
      const questionResult = await this.dataSource.transaction(
        async (manager) => {
      ...
      // Activity logging AFTER successful transaction
      try {
        const totalQuestions = submitQuizDto.total ?? 0;
        const correctAnswers = submitQuizDto.correct ?? 0;
        ...
        if (activityError instanceof Error) {
          this.logger.error(
            `Failed to create quiz activity for user ${submitQuizDto.userId}`,
            activityError.stack,
          );
        } else {
          this.logger.error(
  ...
  async getUserQuizzes(userId: number, isAdmin: boolean): Promise<any> {
    ...
    if (!isAdmin) {
      query.andWhere('quiz.createdBy = :userId', { userId });
    }
  ...
  async getPublishedQuizzes(filters: PublishedQuizFilterDto): Promise<any[]> {
    const query = this.quizRepository
      ...
      .leftJoinAndSelect('quiz.userCreatedBy', 'userCreatedBy')
      ...
      .leftJoin(QuizSubject, 'quizSubjects', 'quizSubjects.quizId = quiz.id')
      .leftJoinAndMapMany(
        'quiz.subjects',
        Subject,
        'subject',
      ...
      // Topic Mapping
      .leftJoin(QuizTopic, 'quizTopics', 'quizTopics.quizId = quiz.id')
  ...
  `,
      ...
      .where('quiz.isPublished = :isPublished', {
        isPublished: true,
      })
  ...
  JOB ROLE FILTER
*/
    if (filters.jobRoleId && Number(filters.jobRoleId) !== 0) {
      query.andWhere('jobRoleSubjects.jobRoleId = :jobRoleId', {
...
*/
    if (filters.mode) {
      query.andWhere('settings.mode = :mode', {
        mode: filters.mode,
      });
    }
  ...
  */
    if (filters.subjectId && Number(filters.subjectId) !== 0) {
      query.andWhere('filterQuizSubjects.subjectId = :subjectId', {
  ...
  */
    if (filters.topicId && Number(filters.topicId) !== 0) {
      query.andWhere(
        `
  ...
  */
    ...
    TOTAL ATTEMPTS
  */
    const attempts = await this.quizResultRepository
      .createQueryBuilder('qr')
      .select('qr.quizId', 'quizId')
      .addSelect('COUNT(qr.id)', 'totalAttempts')
      .where('qr.quizId IN (:...quizIds)', {
        quizIds,
      })
      .groupBy('qr.quizId')
      .getRawMany();
    ...
    for (const item of attempts) {
      attemptMap.set(Number(item.quizId), Number(item.totalAttempts));
    }

    /*
    TOTAL QUESTIONS
  */
    const questionCounts = await this.quizQuestionRepo
      .createQueryBuilder('qq')
      .select('qq.quizId', 'quizId')
      .addSelect('COUNT(qq.id)', 'totalQuestions')
      .where('qq.quizId IN (:...quizIds)', {
        quizIds,
      })
      .groupBy('qq.quizId')
      .getRawMany();
    ...
    for (const item of questionCounts) {
      questionCountMap.set(Number(item.quizId), Number(item.totalQuestions));
    }
  ...
  */
    return quizzes.map((quiz: any) => {
      ...
      return {
        ...
        totalQuestions: questionCountMap.get(quiz.id) || 0,

        totalAttempts: attemptMap.get(quiz.id) || 0,

        settings: quiz.settings || null,

        subjects:
          quiz?.subjects?.map((subject: any) => ({
            id: subject.id,
            title: subject.title,
